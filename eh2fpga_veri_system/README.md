...
```powershell
D:\vivado23\Vivado\2023.2\bin\vivado.bat -mode batch -source scripts\run_board_implementation.tcl
```

最终比特流输出到 `output/board/eh2_veri_system.bit`。脚本会依次完成
`opt_design`、布局、物理优化、布线、实现报告和 `write_bitstream`；若综合
checkpoint 或任何 OOC IP checkpoint 缺失会立即停止。
