...
module eh2_system_controller #(
  ...
  parameter integer TX_COUNT_SYNC_GUARD_CYCLES = 16,
  parameter integer TX_DRAIN_TIMEOUT_CYCLES = 1_000_000
) (
  input  logic clk,
  input  logic resetn,
  ...
  input  logic host_global_reset_pulse,
  input  logic host_info_retransmit_all_pulse,
  ...
  input  logic info_write_done,
  input  logic info_dump_done,
  ...
  logic [31:0] end_tx_target_count;
  logic        end_tx_target_valid;
  logic [30:0] end_tx_wait_counter;
  logic        retransmit_pending;
  ...
  // Completion and submission are independent Gray-counter CDC paths.  Once
  // the synchronized submission target is captured, modular subtraction
  // accepts equality and a completion count which has moved slightly ahead;
  // an older equality-only test could wait forever after a one-count skew.
  wire [31:0] end_tx_completion_delta =
      tx_frame_complete_count - end_tx_target_count;
  wire end_tx_target_reached = end_tx_target_valid &&
                               !end_tx_completion_delta[31];
  ...
  always_ff @(posedge clk) begin
    if (!resetn) begin
      ...
      end_tx_target_count     <= 32'b0;
      end_tx_target_valid     <= 1'b0;
      end_tx_wait_counter     <= 31'b0;
      ...
      // Retain the request even if it arrives while EXE_END is being sent.
      // A complete replay may only begin after the first dump has reached its
      // logical completion state, so the request is consumed in END phase 5.
      if ((state == ST_END) && host_info_retransmit_all_pulse)
      ...
      // The explicit operator reset is accepted in every state.  The reset
      // supervisor stretches this request to 64 controller clocks and each
      // destination clock domain then performs its normal local release.
      if (host_global_reset_pulse) begin
        ...
        program_timer_running   <= 1'b0;
        retransmit_pending      <= 1'b0;
      end else if (fatal_error_pending && (state != ST_ERROR)) begin
        state                   <= ST_ERROR;
        phase                   <= 5'd0;
        waiting_info            <= 1'b0;
        ...
        case (state)
          ...
          ST_EXECUTE: begin
            ...
            case (phase)
              ...
              5'd2: begin
                ...
                if ((eh2_stopped == 2'b11) && info_write_done &&
          ...
          ST_END: begin
            ...
            case (phase)
              ...
              5'd3: begin
                if (info_expected_done) begin
                  // The EXE_END final byte has reached the 125 MHz MAC-facing
                  // stream.  Give both Gray counters time to cross into the
                  // controller domain before snapshotting the final target.
                  reset_counter       <= 8'd0;
                  end_tx_target_valid <= 1'b0;
                  end_tx_wait_counter <= 31'd0;
                  phase <= 5'd5;
                end
              end
              5'd5: begin
                // Do not automatically reset here.  The old design waited on
                // two asynchronous TX frame counters and then reset itself;
                // a missed/late CDC increment could leave the board in an
                // externally visible RESETTING state.  END now retains DDR1
                // until the host has checked its complete log and explicitly
                // sends HOST_GLOBAL_RESET.  A discontinuity request starts a
                // fresh hart0-then-hart1 replay from the retained DDR1 data.
                if (retransmit_pending && !waiting_info) begin
                  // The logical dump may have finished while several frames
                  // are still queued in the 125 MHz TEMAC-facing path.  Take
                  // a synchronized submission snapshot only after its Gray
                  // counter has settled, then wait until the corresponding
                  // physical-frame completion counter reaches that snapshot.
                  // The marker frame is emitted only after this barrier, so
                  // Ethernet ordering makes it an unambiguous boundary between
                  // the old generation and the fresh full replay.
                  if (!end_tx_target_valid) begin
                    if (reset_counter >= TX_COUNT_SYNC_GUARD_CYCLES - 1) begin
                      end_tx_target_count  <= tx_submitted_frame_count;
                      end_tx_target_valid  <= 1'b1;
                      end_tx_wait_counter  <= 31'd0;
                      reset_counter        <= 8'd0;
                    end else begin
                      reset_counter <= reset_counter + 8'd1;
                    end
                  end else if (end_tx_target_reached) begin
                    end_tx_target_valid <= 1'b0;
                    queue_info(MSG_INFO_RETRANSMIT_BEGIN);
                    if (!info_tx_full) begin
                      retransmit_pending <= 1'b0;
                      phase <= 5'd6;
                    end
                  end else if (end_tx_wait_counter >=
                               TX_DRAIN_TIMEOUT_CYCLES - 1) begin
                    // A broken frame-complete CDC must be visible to the host;
                    // never silently start a potentially mixed replay.
                    captured_error_code <= ERR_TXMAC_STREAM;
                    state <= ST_ERROR;
                    phase <= 5'd0;
                    end_tx_target_valid <= 1'b0;
                  end else begin
                    end_tx_wait_counter <= end_tx_wait_counter + 31'd1;
                  end
                end
              end
              5'd6: begin
                if (info_expected_done) begin
                  // The UI completion level is still high from the prior
                  // dump.  Wait for the restarted dump to first deassert it
                  // before accepting its next rising completion level.
                  info_dump_start <= 1'b1;
                  phase <= 5'd7;
                end
              end
          ...
          ST_ERROR: begin
            ...
            if (error_code_already_sent) begin
              // HOST_SEND_STOPPED is an acknowledgement only.  Retain the
              // error state and all diagnostic information until the host
              // explicitly sends HOST_GLOBAL_RESET; no asynchronous TX
              // completion counter is part of the recovery decision.
            end else begin
              if (phase == 5'd0) begin
