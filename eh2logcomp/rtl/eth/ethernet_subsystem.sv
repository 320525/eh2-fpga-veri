...
module ethernet_subsystem #(
  ...
  logic tx_fifo_overflow_raw;
  logic tx_fifo_overflow_sticky_tx;
  logic [0:0] tx_fifo_overflow_ctrl;
  ...
  wire tx_client_resetn_ctrl = hard_resetn && mac_config_done &&
                               !mac_config_error && phy_init_success;
  logic [0:0] tx_client_resetn_tx;
  sync_bits #(.WIDTH(1), .STAGES(3)) tx_reset_release_sync_i (
    .clk(gtx_clk), .resetn(hard_resetn),
    .async_in(tx_client_resetn_ctrl), .sync_out(tx_client_resetn_tx)
  );
  ...
  eth_mac_fifo_block mac_fifo_i (
    ...
    .tx_fifo_clock(gtx_clk), .tx_fifo_resetn(tx_client_resetn_tx[0]),
    ...
    .tx_fifo_status, .tx_fifo_overflow(tx_fifo_overflow_raw),
  ...
  always_ff @(posedge gtx_clk or negedge hard_resetn) begin
    if (!hard_resetn)
      tx_fifo_overflow_sticky_tx <= 1'b0;
    else if (tx_fifo_overflow_raw)
      tx_fifo_overflow_sticky_tx <= 1'b1;
  end
  sync_bits #(.WIDTH(1), .STAGES(3)) tx_overflow_sync_i (
    .clk(ctrl_clk), .resetn(hard_resetn),
    .async_in(tx_fifo_overflow_sticky_tx),
    .sync_out(tx_fifo_overflow_ctrl)
  );
  assign tx_fifo_overflow = tx_fifo_overflow_ctrl[0];
  ...
  // final AXI byte merely enters the client FIFO.  TEMAC may hold
  // tx_statistics_valid high for more than one enabled TX clock; PG051's
  // generated vector decoder therefore treats only its rising edge as one
  // statistics event.  Mirroring that behavior prevents a single frame from
  // advancing the physical-completion count multiple times.
  mac_tx_statistics_counter_cdc tx_complete_count_cdc_i (
    .tx_clk(tx_mac_aclk_unused), .ctrl_clk,
    .resetn(hard_resetn),
    .statistics_valid(tx_statistics_valid_unused),
    .frame_complete_count(tx_frame_complete_count)
  );
endmodule

// TEMAC transmitter-statistics event counter and cumulative CDC.  Keeping the
// cumulative count in Gray code means the 100 MHz control domain cannot lose a
// completed-frame event even though the TX statistics clock is asynchronous.
module mac_tx_statistics_counter_cdc (
  input  logic        tx_clk,
  input  logic        ctrl_clk,
  input  logic        resetn,
  input  logic        statistics_valid,
  output logic [31:0] frame_complete_count
);
  logic        statistics_valid_d;
  logic [31:0] count_tx;
  logic [31:0] count_gray_tx;
  logic [31:0] count_gray_ctrl;

  always_ff @(posedge tx_clk or negedge resetn) begin
    if (!resetn) begin
      statistics_valid_d <= 1'b0;
      count_tx            <= 32'b0;
      count_gray_tx       <= 32'b0;
    end else begin
      statistics_valid_d <= statistics_valid;
      if (statistics_valid && !statistics_valid_d) begin
        count_tx <= count_tx + 32'd1;
        count_gray_tx <= (count_tx + 32'd1) ^
                         ((count_tx + 32'd1) >> 1);
      end
    end
  end

  sync_bits #(.WIDTH(32), .STAGES(3)) tx_complete_count_sync_i (
    .clk(ctrl_clk), .resetn,
    .async_in(count_gray_tx), .sync_out(count_gray_ctrl)
  );

  always_comb begin
    frame_complete_count[31] = count_gray_ctrl[31];
    for (integer bit_index = 30; bit_index >= 0; bit_index = bit_index - 1)
      frame_complete_count[bit_index] =
        frame_complete_count[bit_index+1] ^ count_gray_ctrl[bit_index];
  end
endmodule
