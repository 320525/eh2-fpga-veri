  ...
  always @(posedge clk or negedge resetn) begin
    if (!resetn) begin
      ...
      case (state)
        ...
        // Use 1.00 ns on the PHY-to-MAC receive clock (RX code 3), paired with
        // the FPGA IDELAY maximum of 1.25 ns.  The routed 1.25 ns/code-4 plus
        // 1.10 ns FPGA setting measured +0.594 ns setup and -0.120 ns hold.
        // This combined rebalance moves 0.40 ns into hold and is predicted to
        // retain about +0.19 ns setup and +0.28 ns hold at sign-off.
        ST_DLY_WR_MMDCR:
          launch_mdio(1'b1, detected_phy_addr, REG_MMDCR, 16'h001F,
                      ST_DLY_WR_ADDR);
        ST_DLY_WR_ADDR:
        ...
        ST_DLY_WR_DATA_MODE:
          launch_mdio(1'b1, detected_phy_addr, REG_MMDCR, 16'h401F,
                      ST_DLY_WR_VALUE);
        ST_DLY_WR_VALUE:
          launch_mdio(1'b1, detected_phy_addr, REG_MMDDR, 16'h0073,
                      ST_BMCR_RD_CMD);
