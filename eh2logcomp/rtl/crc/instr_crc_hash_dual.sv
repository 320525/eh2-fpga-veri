...
module instr_crc_hash_dual #(
    ...
    logic [1:0] alloc_atomic_handoff;
    ...
    logic         nb_atomic_handoff [0:1][0:31];
    ...
    always_comb begin
        alloc_accept = '0;
        alloc_atomic_handoff = '0;
        for (integer handoff_hart = 0; handoff_hart < 2;
             handoff_hart = handoff_hart + 1)
            for (integer handoff_rd = 0; handoff_rd <= 31;
                 handoff_rd = handoff_rd + 1)
                nb_atomic_handoff[handoff_hart][handoff_rd] = 1'b0;
        ...
        for (integer alloc_lane = 0; alloc_lane < 2;
            ...
            if (alloc_request[alloc_lane]) begin
                logic alloc_hart;
                logic [4:0] alloc_rd;
                alloc_hart = rv_commit_hart_id[alloc_lane];
                alloc_rd = rv_commit_gpr_rd[alloc_lane];

                // A younger deferred load/div may commit to the same rd in
                // the exact cycle in which EH2 cancels the older unresolved
                // nonblocking writer.  Atomically evict the old zero-result
                // structure into the CRC holding register and let the younger
                // instruction take ownership of nb_struct[hart][rd].  The CRC
                // register may be replaced when its current item is selected
                // in this cycle.  If it is genuinely busy, preserve the old
                // structure and report the normal buffer-conflict error rather
                // than overwrite either instruction.
                alloc_atomic_handoff[alloc_lane] =
                    nb_valid[alloc_hart][alloc_rd] &
                    ~nb_resolved[alloc_hart][alloc_rd] &
                    nb_cancel_match[alloc_hart][alloc_rd] &
                    (~nb_crc_valid[alloc_hart][alloc_rd] |
                     nb_selected[alloc_hart][alloc_rd]);
                ...
                if ((alloc_lane == 1) && alloc_accept[0] &&
                    ...
                    alloc_atomic_handoff[alloc_lane] = 1'b0;
                end
                if (alloc_accept[alloc_lane] &&
                    alloc_atomic_handoff[alloc_lane])
                    nb_atomic_handoff[alloc_hart][alloc_rd] = 1'b1;
            end
        end
        ...
        if (!rst_l) begin
            ...
            for (integer nb_hart = 0; nb_hart < 2; nb_hart = nb_hart + 1) begin
                for (integer nb_rd = 1; nb_rd <= 31; nb_rd = nb_rd + 1) begin
                    ...
                    // During an atomic WAW handoff the unresolved victim still
                    // contains data=0, which is precisely its architectural
                    // result.  Capture that old structure before the allocation
                    // loop below replaces nb_struct with the younger writer.
                    if (nb_atomic_handoff[nb_hart][nb_rd]) begin
                    ...
                    end else if (nb_valid[nb_hart][nb_rd] &&
                                 nb_resolved[nb_hart][nb_rd] &&
                    ...
                    if (nb_valid[nb_hart][nb_rd] &&
                        !nb_resolved[nb_hart][nb_rd] &&
                        !nb_atomic_handoff[nb_hart][nb_rd]) begin
