...
# delay fields to TX code 7 (2.00 ns) and RX code 3 (1.00 ns).
#
...
# delay to 1.00 ns, so both limits move 1.00 ns later.  The prior routed
# code-4/1.25 ns PHY plus 1.10 ns FPGA setting measured +0.594 ns setup and
# -0.120 ns hold.  The final xcvu19p/333.333 MHz BITSLICE DRC permits at most
# 1.10 ns, so receive hold margin must be closed by the legal RXC clock route;
# DELAY_VALUE must not be raised above 1100 ps.
...
set_input_delay 0.000 -max -clock $rgmii_rx_virtual_clock \
  $rgmii_rx_phy_data
set_input_delay -1.000 -min -clock $rgmii_rx_virtual_clock \
  $rgmii_rx_phy_data
set_input_delay 0.000 -max -clock $rgmii_rx_virtual_clock -clock_fall \
  $rgmii_rx_phy_data
set_input_delay -1.000 -min -clock $rgmii_rx_virtual_clock -clock_fall \
  $rgmii_rx_phy_data
