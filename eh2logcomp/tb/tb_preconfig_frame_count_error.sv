...
module preconfig_frame_count_error_case #(
  ...
  logic [31:0] program_end_total_count = 32'b0;
  logic program_first_write_pulse = 1'b0;
  ...
  eh2_system_controller #(
    ...
    .phy_link_up(1'b1), .rgmii_rx_ready(1'b1),
    .mig0_ready(1'b1), .mig1_ready(1'b1),
    .preconfig_program_end_pulse,
    .program_first_write_pulse, .program_end_pulse,
    ...
    .eh2_execute_enable, .prefer_log_tx, .info_dump_start(), .led0,
  ...
  initial begin
    ...
    // end marker with an illegal declared count must enter ERROR and must
    // never start either DDR checker.
    program_frame_count <= ILLEGAL_FRAME_COUNT;
    program_dma_done_count <= ILLEGAL_FRAME_COUNT;
    program_end_total_count <= ILLEGAL_FRAME_COUNT;
    @(posedge clk);
    program_first_write_pulse <= 1'b1;
    @(posedge clk);
    program_first_write_pulse <= 1'b0;

    wait (info_tx_push && (info_tx_code == MSG_PROGRAM_START));
    @(posedge clk);
    info_sent_code <= MSG_PROGRAM_START;
    info_frame_done <= 1'b1;
    @(posedge clk);
    info_frame_done <= 1'b0;
    ...
    wait (info_tx_push && (info_tx_code == MSG_RECEIVE_DONE));
    @(posedge clk);
    info_sent_code <= MSG_RECEIVE_DONE;
    info_frame_done <= 1'b1;
    @(posedge clk);
    info_frame_done <= 1'b0;
    ...
    if (dut.captured_error_code != ERR_PROGRAM_COUNT)
