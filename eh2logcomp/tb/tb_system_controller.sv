...
module tb_system_controller;
  ...
  logic eh2_axi_idle;
  logic info_write_done, info_dump_done;
  ...
  logic prefer_log_tx, info_dump_start, led0;
  ...
  eh2_system_controller #(
    ...
    .TX_COUNT_SYNC_GUARD_CYCLES(4),
    .TX_DRAIN_TIMEOUT_CYCLES(50)
  ) dut (
    .clk, .resetn,
    ...
    .program_dma_busy, .host_send_stopped_pulse, .host_global_reset_pulse,
    ...
    .eh2_started, .eh2_stopped, .eh2_axi_idle,
    .info_write_done, .info_dump_done,
    ...
    .prefer_log_tx, .info_dump_start, .led0,
  ...
  always_ff @(posedge clk) begin
    if (!resetn) begin
      ...
      if (tx_last) begin
        ...
        tx_frame_complete_count <= tx_frame_complete_count +
                                   (completion_counter_lead ? 32'd2 : 32'd1);
    ...
    if (resetn && info_dump_start)
      info_dump_start_count <= info_dump_start_count + 1;
  end
  ...
  initial begin
    ...
    eh2_stopped = 2'b00;
    eh2_axi_idle = 1'b1;
    info_write_done = 1'b0;
    info_dump_done = 1'b0;
    fatal_error_pending = 1'b0;
    fatal_error_code = 32'b0;
    completion_counter_lead = 1'b0;
    ...
    repeat (5) @(posedge clk);
    eh2_stopped[1] <= 1'b1;
    info_write_done <= 1'b1;

    wait_for_state(ST_END);
    wait (info_dump_start);
    info_dump_done <= 1'b1;
    while (frame_count < 14) @(posedge clk);
    // END is a host-verdict hold state.  It must not reset based on
    // asynchronous TX statistics counters after EXE_END.
    repeat (80) @(posedge clk);
    if (global_reset_request || state != ST_END)
      $fatal(1, "END did not retain the completed DDR1 log for host verdict");

    // A retransmit creates a delimiter, restarts the dump, then requires a
    // fresh falling/rising completion transition before returning to END.
    host_info_retransmit_all_pulse <= 1'b1;
    @(posedge clk);
    host_info_retransmit_all_pulse <= 1'b0;
    while (frame_count < 15) @(posedge clk);
    if (observed_codes[14] != MSG_INFO_RETRANSMIT_BEGIN)
      $fatal(1, "retransmit delimiter code missing");
    wait (info_dump_start_count == 2);
    info_dump_done <= 1'b0;
    repeat (8) @(posedge clk);
    info_dump_done <= 1'b1;
    repeat (8) @(posedge clk);
    if (state != ST_END || global_reset_request)
      $fatal(1, "retransmit completion did not return to host-verdict END");
