...
// UI clocks, and uses compact banked storage.  DDR0 preserves a separate
// 1-MiB simulation bank for the 0x8000_0000 program window and the
// 0xA000_0000 random-data window instead of aliasing their low address bits.
// Each bank must exceed the 800,640-byte 200k-instruction image.  The former
// LINE_COUNT=16384 setting provided only 512 KiB per bank after the bit-29
// split, so IFU reads above 0x8008_0000 wrapped to line zero in simulation.
module dual_ddr_mig_wrapper (
  ...
  axi512_memory_model #(
    .LINE_COUNT(32768), .SPLIT_AT_ADDR29(1)
  ) ddr0_memory_i (
  ...
  // 131072 x 64 B = 8 MiB, split into independent 4 MiB windows selected
  // by address bit 32.  This is large enough for both ~100k-record harts in
  // the 200k-instruction system test and preserves the real 0/4-GiB map.
  axi512_memory_model #(
    .LINE_COUNT(131072), .SPLIT_AT_ADDR32(1)
  ) ddr1_memory_i (
