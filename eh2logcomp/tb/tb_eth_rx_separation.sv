...
module tb_eth_rx_separation;
  ...
  logic program_end_pulse, host_send_stopped_pulse, host_global_reset_pulse;
  ...
  system_info_rx_decoder decoder_i (
    ...
    .host_send_stopped_pulse, .host_global_reset_pulse,
    .host_info_retransmit_all_pulse, .malformed_frame
  );
  ...
  integer retransmit_pulse_count;
  ...
  always_ff @(posedge clk) begin
    if (!resetn) begin
      ...
      if (host_global_reset_pulse)
        reset_pulse_count <= reset_pulse_count + 1;
      if (host_info_retransmit_all_pulse)
        retransmit_pulse_count <= retransmit_pulse_count + 1;
      if (program_accepted)
  ...
  task automatic send_host_global_reset_frame;
    begin
      ...
      // HOST_GLOBAL_RESET 0x44_13_44_45 followed by 42 zero bytes.
      send_word(16'h1344, 1'b0);
  ...
  task automatic send_host_retransmit_frame;
    begin
      send_word(16'h3202, 1'b0);
      send_word(16'h2505, 1'b0);
      send_word(16'hff00, 1'b0);
      ...
      send_word(16'h6655, 1'b0);
      send_word(16'hb588, 1'b0);
      // HOST_INFO_RETRANSMIT_ALL 0x44_14_44_45 plus 42 zero bytes.
      send_word(16'h1444, 1'b0);
      send_word(16'h4544, 1'b0);
      for (integer i = 9; i < 30; i = i + 1)
        send_word(16'h0000, i == 29);
    end
  ...
  initial begin
    ...
    send_host_global_reset_frame();
    wait (reset_pulse_count == 1);
    if ((end_pulse_count != 1) || (stopped_pulse_count != 1))
      $fatal(1, "host global reset aliased to another system command");

    send_host_retransmit_frame();
    wait (retransmit_pulse_count == 1);
    if ((end_pulse_count != 1) || (stopped_pulse_count != 1) ||
        (reset_pulse_count != 1))
      $fatal(1, "host retransmit aliased to another system command");

    send_unrelated_frame();
    repeat (20) @(posedge clk);
    if ((accepted_program_count != 1) || (accepted_info_count != 4) ||
