...
module tb_error_global_reset_flow;
  ...
  logic info_fifo_h1_error = 1'b0;
  logic info_queue_h1_error = 1'b0;
  logic info_capture_h1_error = 1'b0;
  logic fatal_pending;
  ...
  logic host_global_reset_pulse = 1'b0;
  logic host_info_retransmit_all_pulse = 1'b0;
  ...
  system_error_monitor monitor_i (
    ...
    .err_info_fifo_h0(1'b0), .err_info_fifo_h1(info_fifo_h1_error),
    .err_info_queue_h0(1'b0), .err_info_queue_h1(info_queue_h1_error),
    .err_info_capture_h0(1'b0),
    .err_info_capture_h1(info_capture_h1_error),
  ...
  eh2_system_controller #(
    ...
    .fatal_error_pending(fatal_pending),
  ...
  task automatic acknowledge_then_explicit_reset;
    integer low_cycles;
    begin
      ...
      if (global_reset_request || !hard_resetn)
        $fatal(1, "HOST_SEND_STOPPED incorrectly initiated an automatic reset");
  ...
  initial begin
    ...
    acknowledge_then_explicit_reset();

    // A MAC FCS error likewise holds diagnostics until explicit host reset.
    fcs_error <= 1'b1;
    @(posedge clk);
    fcs_error <= 1'b0;
    ...
    // The three hart1 information stages must remain distinguishable at the
    // host.  C2 is reserved for an actual XPM write overflow; D2 and D4 name
    // the upstream elastic queue and capture stage respectively.
    info_fifo_h1_error <= 1'b1;
    @(posedge clk);
    info_fifo_h1_error <= 1'b0;
    complete_error_frame(ERR_INFO_FIFO_H1);
    acknowledge_then_explicit_reset();

    info_queue_h1_error <= 1'b1;
    @(posedge clk);
    info_queue_h1_error <= 1'b0;
    complete_error_frame(ERR_INFO_QUEUE_H1);
    acknowledge_then_explicit_reset();

    info_capture_h1_error <= 1'b1;
    @(posedge clk);
    info_capture_h1_error <= 1'b0;
    complete_error_frame(ERR_INFO_CAPTURE_H1);
    acknowledge_then_explicit_reset();

    // The WebUI reset button bypasses the error acknowledgement handshake and
    // requests the same stretched global reset from any normal state.
    host_global_reset_pulse <= 1'b1;
    @(posedge clk);
    host_global_reset_pulse <= 1'b0;
    wait (!hard_resetn);
    wait (hard_resetn);
    if (state != ST_PRECONFIG)
      $fatal(1, "operator reset did not return controller to PRECONFIG");

    $display("TB_PASS error handshake and operator global reset");
    $finish;
  end
