...
function Run-Xsim {
  ...
  $log = "${Top}_xsim.log"
  Invoke-Step "$vivadoBin\xsim.bat" @("${Top}_sim", '-runall', '-log', $log)
  if (Select-String -Path $log -Pattern '^Fatal:' -Quiet) {
...
Run-Xsim 'tb_mac_tx_statistics_counter_cdc' @(
  'rtl\common\sync_bits.sv',
  'rtl\eth\ethernet_subsystem.sv',
  'tb\tb_mac_tx_statistics_counter_cdc.sv'
)
