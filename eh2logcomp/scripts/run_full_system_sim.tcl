...
set compile_only [expr {[lsearch -exact $argv "compile_only"] >= 0}]
set riscv_dv_10k [expr {[lsearch -exact $argv "riscvdv_10k"] >= 0}]
set board_log_repro [expr {[lsearch -exact $argv "board_log_repro"] >= 0}]

set_param general.maxThreads 8
open_project $project_file

# The production capture implementation is split into one independently
# placeable bank per hart.  Older project files predate that split, so add the
# two files here as normal design sources before either simulation or a later
# synthesis run.  add_files is guarded to keep repeated regressions idempotent.
set capture_hierarchy_sources [list \
  [file join $root_dir rtl info instr_info_capture_hart_bank.sv] \
  [file join $root_dir rtl info instr_info_capture_dual_hier.sv] \
  [file join $root_dir rtl info info_record_elastic_4w4r.sv] \
  [file join $root_dir rtl info info_dma_data_elastic.sv] \
  [file join $root_dir rtl info info_fifo_pingpong_4w2r.sv] \
  [file join $root_dir rtl ddr info_ddr_pingpong_write_dma.sv]]
foreach source $capture_hierarchy_sources {
...
set simulation_include_dirs [list \
  [file join $reference_log config default_mt] \
  [file join $reference_root design]]
if {$board_log_repro} {
  lappend simulation_include_dirs \
    [file join $root_dir programs board_log_repro build]
  set_property verilog_define {BOARD_LOG_REPRO_TOP DIRECT_DDR_PRELOAD} [get_filesets sim_1]
} elseif {$riscv_dv_10k} {
  lappend simulation_include_dirs \
    [file join $root_dir programs riscvdv_10k_top build]
  set_property verilog_define {RISCV_DV_10K_TOP DIRECT_DDR_PRELOAD} [get_filesets sim_1]
} else {
  lappend simulation_include_dirs \
    [file join $root_dir programs stress_200k_dualhart_system build]
  # The full 200k regression preloads the exact program payload into DDR0
  # after READY.  It validates execution, the new four-bank Info FIFO, DDR1
  # DMA and outgoing Ethernet frames without spending simulation time on the
  # already separately verified RGMII program-ingress waveform.
  set_property verilog_define {DIRECT_DDR_PRELOAD} [get_filesets sim_1]
}
set_property include_dirs $simulation_include_dirs [get_filesets sim_1]
