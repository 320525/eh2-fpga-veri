...
module tb_eh2_stress_200k_dualhart;
    logic core_clk = 1'b0;
    ...
    eh2_crc_soc #(
        .MEM_FILE("../programs/stress_200k_dualhart/stress_200k_dualhart.mem64"),
        .RUN_HART_MASK(2'b11),
    ...
    initial begin : run_test
        integer h;
        integer p;
        // EH2 normally starts hart1 only after hart0 writes the proprietary
        // MHARTSTART CSR.  The exact same ELF must also run on unmodified
        // Spike, where that CSR does not exist, so the testbench models the
        // platform boot controller by setting the internal hart-start flop
        // once after reset. Its normal self-hold path retains the value.
        // This changes only pre-simulation boot state, not commit/CRC logic.
        struct_log = $fopen("../sim/stress_200k_actual_structs.txt", "w");
        result_log = $fopen("../sim/stress_200k_results.txt", "w");
        div_timing_log = $fopen("../sim/stress_200k_div_timing.txt", "w");
        for (h = 0; h < 2; h = h + 1)
        ...
        wait (core_rst_l);
        @(negedge core_clk);
        dut.core_i.veer.dec.tlu.mhartstart_ff.dout = 1'b1;
        ...
        $display("EH2_STRESS_200K_PASS hart0=%0d hart1=%0d packages=%0d/%0d,%0d/%0d",
                 generated_count[0], generated_count[1],
