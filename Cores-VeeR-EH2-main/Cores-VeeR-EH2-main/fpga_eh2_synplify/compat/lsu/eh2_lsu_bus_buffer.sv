...
`include "eh2_param.vh"
   ...
   assign obuf_tag0_in[pt.LSU_BUS_TAG-1:0] = ibuf_buf_byp ? 4'(WrPtr0_dc5) : 4'(CmdPtr0);
   assign obuf_tag1_in[pt.LSU_BUS_TAG-1:0] = ibuf_buf_byp ? 4'(WrPtr1_dc5) : 4'(CmdPtr1);
         ...
         case (buf_state[i])
            ...
            RESP: begin
                     ...
                     buf_error_en[i]           = buf_state_bus_en[i] & lsu_bus_clk_en & ((bus_rsp_read_error  & (bus_rsp_read_tid == tid)  & (bus_rsp_read_tag  == 4'(i))) |
                                                                                         ...
                                                                                         (bus_rsp_write_error & (bus_rsp_write_tid == tid) & pt.BUILD_AXI_NATIVE & (bus_rsp_write_tag == 4'(i))));
   ...
   always_comb begin
      ...
      for (int i=0; i<DEPTH; i++) begin
         bus_addr_match_pending |= (pt.BUILD_AXI_NATIVE & obuf_valid & (obuf_addr[31:3] == buf_addr[i][31:3]) & (buf_state[i] == RESP) & ~((obuf_tag0 == 4'(i)) | (obuf_merge & (obuf_tag1 == 4'(i)))));
      end
   end
