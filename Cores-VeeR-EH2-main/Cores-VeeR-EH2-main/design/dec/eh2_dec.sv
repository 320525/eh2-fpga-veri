...
`include "eh2_param.vh"
  ...
  (
   ...
   // Architectural commit monitor. Lane 0 is i0; lane 1 is i1.
   ...
   // Final nonblocking GPR write ports after all WAW cancellation.
   output logic             rv_nb_load_gpr_wen,
   output logic             rv_nb_load_gpr_hart_id,
   output logic [4:0]       rv_nb_load_gpr_rd,
   output logic [31:0]      rv_nb_load_gpr_wdata,
   output logic             rv_nb_div_gpr_wen,
   output logic             rv_nb_div_gpr_hart_id,
   output logic [4:0]       rv_nb_div_gpr_rd,
   output logic [31:0]      rv_nb_div_gpr_wdata,
...
// Debug end
   ...
   logic [pt.NUM_THREADS-1:0] dec_tlu_i0_commit_wb, dec_tlu_i1_commit_wb;
...
// multithreaded signals
  ...
  for (genvar i=0; i<pt.NUM_THREADS; i++) begin : ib
   ...
   assign rv_nb_load_gpr_wen     = |dec_nonblock_load_wen;
   assign rv_nb_load_gpr_hart_id = lsu_nonblock_load_data_tid;
   assign rv_nb_load_gpr_wdata   = lsu_nonblock_load_data;

   always_comb begin
      rv_nb_load_gpr_rd = '0;
      for (int unsigned i=0; i<pt.NUM_THREADS; i++) begin
         if (dec_nonblock_load_wen[i])
           rv_nb_load_gpr_rd = dec_nonblock_load_waddr[i];
      end
   end

   assign rv_nb_div_gpr_wen      = exu_div_wren;
   assign rv_nb_div_gpr_hart_id  = div_tid_wb;
   assign rv_nb_div_gpr_rd       = div_waddr_wb;
   assign rv_nb_div_gpr_wdata    = exu_div_result;
