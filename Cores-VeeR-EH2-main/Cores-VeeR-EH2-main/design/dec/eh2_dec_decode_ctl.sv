...
`include "eh2_param.vh"
  ...
  (
   ...
   // Architectural commit monitor. All fields are aligned at EH2 WB commit.
   output logic [1:0]       rv_commit_valid,
   output logic [1:0][31:0] rv_commit_insn,
   output logic [1:0][31:0] rv_commit_pc,
   output logic [1:0]       rv_commit_hart_id,
   output logic [1:0][1:0]  rv_commit_priv_mode,
   output logic [1:0]       rv_commit_gpr_wen_intent,
   output logic [1:0]       rv_commit_gpr_wen,
   output logic [1:0][4:0]  rv_commit_gpr_rd,
   output logic [1:0][31:0] rv_commit_gpr_wdata,
   output logic [1:0]       rv_commit_csr_wen,
   output logic [1:0][11:0] rv_commit_csr_addr,
   output logic [1:0][31:0] rv_commit_csr_wdata,
   output logic [1:0]       rv_commit_is_nonblock,
   output logic [1:0]       rv_commit_is_nonblock_load,
   output logic [1:0]       rv_commit_is_nonblock_div,
   output logic [1:0]       rv_commit_waw_victim,

   output logic [1:0]       rv_nb_waw_valid,
   output logic [1:0][31:0] rv_nb_waw_victim_insn,
   output logic [1:0][31:0] rv_nb_waw_victim_pc,
   output logic [1:0]       rv_nb_waw_victim_hart_id,
   output logic [1:0][4:0]  rv_nb_waw_victim_gpr_rd,
   output logic [1:0]       rv_nb_waw_victim_is_load,
   output logic [1:0]       rv_nb_waw_victim_is_div,

   input logic [pt.NUM_THREADS-1:0] dec_tlu_i0_commit_wb,
   input logic [pt.NUM_THREADS-1:0] dec_tlu_i1_commit_wb,
...
// end CSR interface
   ...
   logic [pt.NUM_THREADS-1:0]       cam_i0_nb_waw;
   logic [pt.NUM_THREADS-1:0]       cam_i1_nb_waw;
   logic [pt.NUM_THREADS-1:0][31:0] cam_i0_nb_waw_victim_insn;
   logic [pt.NUM_THREADS-1:0][31:0] cam_i1_nb_waw_victim_insn;
   logic [pt.NUM_THREADS-1:0][31:1] cam_i0_nb_waw_victim_pc;
   logic [pt.NUM_THREADS-1:0][31:1] cam_i1_nb_waw_victim_pc;
   logic [pt.NUM_THREADS-1:0][4:0]  cam_i0_nb_waw_victim_rd;
   logic [pt.NUM_THREADS-1:0][4:0]  cam_i1_nb_waw_victim_rd;
   ...
   logic i0_same_cycle_waw_wb;
   logic i0_nbdiv_waw_wb, i1_nbdiv_waw_wb;
   logic [31:0] div_victim_inst;
   logic [31:1] div_victim_pc;

   typedef struct packed {
      logic [1:0]       valid;
      logic [1:0]       hart_id;
      logic [1:0]       gpr_wen_intent;
      logic [1:0]       gpr_wen;
      logic [1:0][4:0]  gpr_rd;
      logic [1:0][31:0] gpr_wdata;
      logic [1:0]       csr_wen;
      logic [1:0][11:0] csr_addr;
      logic [1:0][31:0] csr_wdata;
      logic [1:0]       is_nonblock_load;
      logic [1:0]       is_nonblock_div;
      logic [1:0]       waw_victim;
      logic [1:0]       nb_waw_valid;
      logic [1:0][31:0] nb_waw_victim_insn;
      logic [1:0][31:1] nb_waw_victim_pc;
      logic [1:0]       nb_waw_victim_hart_id;
      logic [1:0][4:0]  nb_waw_victim_gpr_rd;
      logic [1:0]       nb_waw_victim_is_load;
      logic [1:0]       nb_waw_victim_is_div;
   } eh2_commit_monitor_pkt_t;

   eh2_commit_monitor_pkt_t commit_monitor_wb;
...
// START: non block load cam logic
   ...
   for (genvar i=0; i<pt.NUM_THREADS; i++) begin : cam
      ...
      eh2_dec_cam #(.pt(pt)) cam (
                        ...
                        .i0_nb_waw                (cam_i0_nb_waw[i]             ),
                        .i1_nb_waw                (cam_i1_nb_waw[i]             ),
                        .i0_nb_waw_victim_insn    (cam_i0_nb_waw_victim_insn[i]),
                        .i1_nb_waw_victim_insn    (cam_i1_nb_waw_victim_insn[i]),
                        .i0_nb_waw_victim_pc      (cam_i0_nb_waw_victim_pc[i]  ),
                        .i1_nb_waw_victim_pc      (cam_i1_nb_waw_victim_pc[i]  ),
                        .i0_nb_waw_victim_rd      (cam_i0_nb_waw_victim_rd[i]  ),
                        .i1_nb_waw_victim_rd      (cam_i1_nb_waw_victim_rd[i]  ),
...
// end tlu stuff
   ...
   // I0 is the older lane. Normally its GPR write is suppressed by the
   // younger I1 write. If I1 is itself a deferred nonblocking load, a normal
   // I0 write is still architecturally visible; only an I0 nonblocking
   // load/divide is canceled in that corner case. Include those explicit CAM
   // and divider cancellation cases so every same-cycle victim is reported.
   assign i0_same_cycle_waw_wb = wbd.i0v & wbd.i1v &
                                  (wbd.i0rd[4:0] == wbd.i1rd[4:0]) &
                                  (wbd.i0tid == wbd.i1tid) &
                                  ~dec_tlu_i0_kill_writeb_wb &
                                  ~dec_tlu_i1_kill_writeb_wb &
                                  (~cam_i1_load_kill_wen[wbd.i1tid] |
                                   (wbd.i0load & cam_i0_load_kill_wen[wbd.i0tid]) |
                                   (wbd.i0div & div_valid));
...
// cancel if any younger inst committing this cycle to same dest as nonblock divide
   ...
   // These terms cover both a write before divide data returns and a write
   // in the divide-return cycle. The two timing cases intentionally share one
   // architectural nonblocking-WAW event.
   assign i0_nbdiv_waw_wb = div_valid & ~div_e1_to_wb &
                            (wbd.i0rd[4:0] == div_rd[4:0]) &
                            (wbd.i0tid == div_tid) & i0_wen_wb;
   assign i1_nbdiv_waw_wb = div_valid & ~div_e1_to_wb &
                            (wbd.i1rd[4:0] == div_rd[4:0]) &
                            (wbd.i1tid == div_tid) & i1_wen_wb;

   rvdffe #(63) div_commit_meta_ff (.*, .clk(free_l2clk),
                                     .en(wbd.i0div & wbd.i0valid & ~dec_tlu_i0_kill_writeb_wb),
                                     .din({i0_inst_wb[31:0], i0_pc_wb[31:1]}),
                                     .dout({div_victim_inst[31:0], div_victim_pc[31:1]}));
   ...
   // Commit metadata must remain available even when the legacy trace port is
   // disabled. Keep the existing instruction/PC pipeline live for this monitor.
   ...
   // Build the monitor packet directly at EH2's architectural WB commit
   // point.  The CRC tracker must allocate the sequence number and any
   // nonblocking entry in this cycle, not one cycle later at WB+1.
   always_comb begin
      commit_monitor_wb = '0;

      commit_monitor_wb.valid[0] = |dec_tlu_i0_commit_wb;
      commit_monitor_wb.valid[1] = |dec_tlu_i1_commit_wb;
      commit_monitor_wb.hart_id = {wbd.i1tid, wbd.i0tid};

      commit_monitor_wb.gpr_wen_intent[0] = wbd.i0v & ~dec_tlu_i0_kill_writeb_wb;
      commit_monitor_wb.gpr_wen_intent[1] = wbd.i1v & ~dec_tlu_i1_kill_writeb_wb;
      commit_monitor_wb.gpr_wen = {dec_i1_wen_wb, dec_i0_wen_wb};
      commit_monitor_wb.gpr_rd = {wbd.i1rd[4:0], wbd.i0rd[4:0]};
      commit_monitor_wb.gpr_wdata = {dec_i1_wdata_wb[31:0], dec_i0_wdata_wb[31:0]};

      commit_monitor_wb.csr_wen[0] = dec_i0_csr_wen_wb;
      commit_monitor_wb.csr_addr[0] = dec_i0_csr_wraddr_wb[11:0];
      commit_monitor_wb.csr_wdata[0] = dec_i0_csr_wrdata_wb[31:0];

      commit_monitor_wb.is_nonblock_load[0] = cam_i0_load_kill_wen[wbd.i0tid] & wbd.i0load;
      commit_monitor_wb.is_nonblock_load[1] = cam_i1_load_kill_wen[wbd.i1tid] & wbd.i1load;
      commit_monitor_wb.is_nonblock_div[0] = wbd.i0div;
      commit_monitor_wb.waw_victim[0] = i0_same_cycle_waw_wb;

      commit_monitor_wb.nb_waw_valid[0] = (|cam_i0_nb_waw) | i0_nbdiv_waw_wb;
      commit_monitor_wb.nb_waw_valid[1] = (|cam_i1_nb_waw) | i1_nbdiv_waw_wb;
      commit_monitor_wb.nb_waw_victim_hart_id[0] = wbd.i0tid;
      commit_monitor_wb.nb_waw_victim_hart_id[1] = wbd.i1tid;
      commit_monitor_wb.nb_waw_victim_is_load[0] = |cam_i0_nb_waw;
      commit_monitor_wb.nb_waw_victim_is_load[1] = |cam_i1_nb_waw;
      commit_monitor_wb.nb_waw_victim_is_div = {i1_nbdiv_waw_wb, i0_nbdiv_waw_wb};

      for (int t=0; t<pt.NUM_THREADS; t++) begin
         commit_monitor_wb.nb_waw_victim_insn[0] |= cam_i0_nb_waw_victim_insn[t];
         commit_monitor_wb.nb_waw_victim_insn[1] |= cam_i1_nb_waw_victim_insn[t];
         commit_monitor_wb.nb_waw_victim_pc[0] |= cam_i0_nb_waw_victim_pc[t];
         commit_monitor_wb.nb_waw_victim_pc[1] |= cam_i1_nb_waw_victim_pc[t];
         commit_monitor_wb.nb_waw_victim_gpr_rd[0] |= cam_i0_nb_waw_victim_rd[t];
         commit_monitor_wb.nb_waw_victim_gpr_rd[1] |= cam_i1_nb_waw_victim_rd[t];
      end

      if (i0_nbdiv_waw_wb) begin
         commit_monitor_wb.nb_waw_victim_insn[0] |= div_victim_inst;
         commit_monitor_wb.nb_waw_victim_pc[0] |= div_victim_pc;
         commit_monitor_wb.nb_waw_victim_gpr_rd[0] |= div_rd;
      end
      if (i1_nbdiv_waw_wb) begin
         commit_monitor_wb.nb_waw_victim_insn[1] |= div_victim_inst;
         commit_monitor_wb.nb_waw_victim_pc[1] |= div_victim_pc;
         commit_monitor_wb.nb_waw_victim_gpr_rd[1] |= div_rd;
      end
   end

   assign rv_commit_valid = commit_monitor_wb.valid;
   assign rv_commit_insn[0] = i0_inst_wb[31:0];
   assign rv_commit_insn[1] = i1_inst_wb[31:0];
   assign rv_commit_pc[0] = {i0_pc_wb[31:1], 1'b0};
   assign rv_commit_pc[1] = {i1_pc_wb[31:1], 1'b0};
   assign rv_commit_hart_id = commit_monitor_wb.hart_id;
   assign rv_commit_priv_mode[0] = 2'b11;
   assign rv_commit_priv_mode[1] = 2'b11;
   assign rv_commit_gpr_wen_intent = commit_monitor_wb.gpr_wen_intent;
   assign rv_commit_gpr_wen = commit_monitor_wb.gpr_wen;
   assign rv_commit_gpr_rd = commit_monitor_wb.gpr_rd;
   assign rv_commit_gpr_wdata = commit_monitor_wb.gpr_wdata;
   assign rv_commit_csr_wen = commit_monitor_wb.csr_wen;
   assign rv_commit_csr_addr = commit_monitor_wb.csr_addr;
   assign rv_commit_csr_wdata = commit_monitor_wb.csr_wdata;
   assign rv_commit_is_nonblock_load = commit_monitor_wb.is_nonblock_load & rv_commit_valid;
   assign rv_commit_is_nonblock_div = commit_monitor_wb.is_nonblock_div & rv_commit_valid;
   assign rv_commit_is_nonblock = rv_commit_is_nonblock_load | rv_commit_is_nonblock_div;
   assign rv_commit_waw_victim = commit_monitor_wb.waw_victim & rv_commit_valid;

   assign rv_nb_waw_valid = commit_monitor_wb.nb_waw_valid & rv_commit_valid;
   assign rv_nb_waw_victim_insn = commit_monitor_wb.nb_waw_victim_insn;
   assign rv_nb_waw_victim_pc[0] = {commit_monitor_wb.nb_waw_victim_pc[0], 1'b0};
   assign rv_nb_waw_victim_pc[1] = {commit_monitor_wb.nb_waw_victim_pc[1], 1'b0};
   assign rv_nb_waw_victim_hart_id = commit_monitor_wb.nb_waw_victim_hart_id;
   assign rv_nb_waw_victim_gpr_rd = commit_monitor_wb.nb_waw_victim_gpr_rd;
   assign rv_nb_waw_victim_is_load = commit_monitor_wb.nb_waw_victim_is_load;
   assign rv_nb_waw_victim_is_div = commit_monitor_wb.nb_waw_victim_is_div;

`ifdef RV_ASSERT_ON
   assert_i0_nb_waw_single_victim_type: assert #0 (~((|cam_i0_nb_waw) & i0_nbdiv_waw_wb));
   assert_i1_nb_waw_single_victim_type: assert #0 (~((|cam_i1_nb_waw) & i1_nbdiv_waw_wb));
`endif
...
`include "eh2_param.vh"
   ...
   input logic [31:0] i0_inst_wb,
   input logic [31:0] i1_inst_wb,
   input logic [31:1] i0_pc_wb,
   input logic [31:1] i1_pc_wb,
   ...
   output logic       i0_nb_waw,
   output logic       i1_nb_waw,
   output logic [31:0] i0_nb_waw_victim_insn,
   output logic [31:0] i1_nb_waw_victim_insn,
   output logic [31:1] i0_nb_waw_victim_pc,
   output logic [31:1] i1_nb_waw_victim_pc,
   output logic [4:0]  i0_nb_waw_victim_rd,
   output logic [4:0]  i1_nb_waw_victim_rd,
   ...
   eh2_load_cam_commit_pkt_t [NBLOAD_SIZE_MSB:0] cam_commit;
   eh2_load_cam_commit_pkt_t [NBLOAD_SIZE_MSB:0] cam_commit_in;
   eh2_load_cam_commit_pkt_t [NBLOAD_SIZE_MSB:0] cam_commit_raw;
...
`endif
   ...
   for (genvar i=0; i<NBLOAD_SIZE; i++) begin : cam_array
      ...
      always_comb begin
         ...
         cam[i] = cam_raw[i];
         cam_commit[i] = cam_commit_raw[i];
         cam_commit_in[i] = cam_commit[i];
         ...
         if (nonblock_load_valid_wb & (lsu_nonblock_load_inv_tag_dc5[NBLOAD_TAG_MSB:0]==cam[i].tag[NBLOAD_TAG_MSB:0]) & cam[i].valid)
           begin
              cam_in[i].wb = 1'b1;
              if (wbd.i0load & (wbd.i0tid == tid)) begin
                 cam_commit_in[i].insn = i0_inst_wb;
                 cam_commit_in[i].pc   = i0_pc_wb;
              end
              else begin
                 cam_commit_in[i].insn = i1_inst_wb;
                 cam_commit_in[i].pc   = i1_pc_wb;
              end
           end
      ...
      rvdffie #( $bits(eh2_load_cam_commit_pkt_t) ) cam_commit_ff (.*, .din(cam_commit_in[i]), .dout(cam_commit_raw[i]));
   ...
   // Detect a younger committed write that invalidates an older committed
   // nonblocking-load CAM entry. Use cam_raw so a simultaneous load return
   // cannot hide the WAW victim before this event is formed.
   always_comb begin
      i0_nb_waw = 1'b0;
      i1_nb_waw = 1'b0;
      i0_nb_waw_victim_insn = '0;
      i1_nb_waw_victim_insn = '0;
      i0_nb_waw_victim_pc = '0;
      i1_nb_waw_victim_pc = '0;
      i0_nb_waw_victim_rd = '0;
      i1_nb_waw_victim_rd = '0;

      for (int i=0; i<NBLOAD_SIZE; i++) begin
         i0_nb_waw |= i0_wen_wb & (wbd.i0tid == tid) & cam_raw[i].valid & cam_raw[i].wb &
                      (wbd.i0rd[4:0] == cam_raw[i].rd[4:0]);
         i1_nb_waw |= i1_wen_wb & (wbd.i1tid == tid) & cam_raw[i].valid & cam_raw[i].wb &
                      (wbd.i1rd[4:0] == cam_raw[i].rd[4:0]);

         i0_nb_waw_victim_insn |= cam_commit_raw[i].insn &
            {32{i0_wen_wb & (wbd.i0tid == tid) & cam_raw[i].valid & cam_raw[i].wb &
                (wbd.i0rd[4:0] == cam_raw[i].rd[4:0])}};
         i1_nb_waw_victim_insn |= cam_commit_raw[i].insn &
            {32{i1_wen_wb & (wbd.i1tid == tid) & cam_raw[i].valid & cam_raw[i].wb &
                (wbd.i1rd[4:0] == cam_raw[i].rd[4:0])}};
         i0_nb_waw_victim_pc |= cam_commit_raw[i].pc &
            {31{i0_wen_wb & (wbd.i0tid == tid) & cam_raw[i].valid & cam_raw[i].wb &
                (wbd.i0rd[4:0] == cam_raw[i].rd[4:0])}};
         i1_nb_waw_victim_pc |= cam_commit_raw[i].pc &
            {31{i1_wen_wb & (wbd.i1tid == tid) & cam_raw[i].valid & cam_raw[i].wb &
                (wbd.i1rd[4:0] == cam_raw[i].rd[4:0])}};
         i0_nb_waw_victim_rd |= cam_raw[i].rd &
            {5{i0_wen_wb & (wbd.i0tid == tid) & cam_raw[i].valid & cam_raw[i].wb &
               (wbd.i0rd[4:0] == cam_raw[i].rd[4:0])}};
         i1_nb_waw_victim_rd |= cam_raw[i].rd &
            {5{i1_wen_wb & (wbd.i1tid == tid) & cam_raw[i].valid & cam_raw[i].wb &
               (wbd.i1rd[4:0] == cam_raw[i].rd[4:0])}};
      end
   end
