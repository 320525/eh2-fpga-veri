...
module rx_fifo_frame_ctrl (
  ...
  always @(posedge clk) begin
    if (!resetn) begin
      ...
      dest_match         <= 1'b1;
      frame_count        <= 32'd0;
      // The first frame starts at address zero.  Each completed frame moves
      // the next command start address forward by exactly 1024 bytes.
      dma_write_addr     <= 32'h0000_0000;
      frame_done         <= 1'b0;
      dma_done           <= 1'b0;
      dma_error          <= 1'b0;
